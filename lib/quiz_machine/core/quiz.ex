defmodule QuizMachine.Core.Quiz do
  ...
  mastered `Question`-s.
  """
  alias QuizMachine.Core.{Template, Question, Response}
  ...
  def answer_question(quiz, %Response{correct_answer: true} = response) do
    new_quiz =
      quiz
      |> inc_record()
  ...
  def answer_question(quiz, %Response{correct_answer: false} = response) do
    quiz
    |> reset_record()
    |> save_response(response)
  end
